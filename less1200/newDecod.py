...
def recognize(filename, features, fst_graph, thr_common, lm_scale, wd_add):
            ...
            if token.is_alive != False:
                ...
                for arc_id in range(id1,id2):
                    ...
                    if wfst.invertSymbolTable(fst_graph.InputSymbols)[arc.ilabel] != '<eps>':
                        ...
                        if type(fst_graph.Final(token.state)) == int or float:
                            new_token.dist += fst_graph.Final(arc.nextstate) * lm_scale
                        if wfst.invertSymbolTable(fst_graph.OutputSymbols)[arc.olabel] != '<eps>':
                            if token.state != new_token.state:
                                new_token.dist += wd_add
                                new_token.sentence += wfst.invertSymbolTable(fst_graph.OutputSymbols)[arc.olabel] + ' '
    ...
    if len(active_tokens) != 0:
        ...
        print("FINAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAL!!!!  ", best_token_ol.dist, best_token_ol.sentence)
    else:
