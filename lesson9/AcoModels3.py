...
# python3 trainAM2.py dan3 out_model2 5
# python3 Task1.py

class AcoModel:
    def dist(self, ftrInput, t=0):
    #вычисление дистанции
        return -4.0 * self.gmm.loglikelyhood(ftrInput)


    #training
    def __init__(self, fnSortFile, num_of_mixes, p): #to train model
        self.name = os.path.basename(fnSortFile).replace('.npy','') #remove extension # Возвращает базовое имя пути
        # means = np.mean(np.load(fnSortFile), axis=0)
        # vars = np.var(np.load(fnSortFile), axis=0)
        # self.gmm = gmm.DiagGauss(means=means, vars=vars)
...
class AcoModelSet:
    ...
    def __init__(self, sortDir, saveToFile, num_of_mixes):
        ...
        for fnModel in os.listdir(sortDir): # Возвращает название файлов директории
            model = AcoModel(os.path.join(sortDir, fnModel), num_of_mixes, p) # Соеденяем имя файла и путь, вызываем AcoModel
    ...
    def show(self):
        for name, model in self.name2model.items(): # Выводит имя и среднее значение
            print("Name: {}; num_of_average: {}".format(name, model.num_of_average))
